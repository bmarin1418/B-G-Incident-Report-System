...
(function(a, b, c) {
    ...
    e.extend = function(a) {
        ...
        collectFormValues: function(a, b) {
            ...
            for (b = b || {}, g = a.querySelectorAll("input[name], textarea[name]"), c = 0; c < g.length; ++c) f = g.item(c), e.isDefined(f.getAttribute("data-ignored")) || (i = e.sanitizeFormValue(f.value, b), "number" === f.type ? i = i ? +i : null : "checkbox" === f.type ? f.attributes.value ? f.checked || (i = j[f.name] || null) : i = f.checked : "radio" === f.type && (f.checked || (i = j[f.name] || null)), j[f.name] = i);
            for (g = a.querySelectorAll("select[name]"), c = 0; c < g.length; ++c) {
                if (f = g.item(c), f.multiple) {
                    i = [];
                    for (d in f.options) h = f.options[d], h.selected && i.push(e.sanitizeFormValue(h.value, b))
                } else i = e.sanitizeFormValue(f.options[f.selectedIndex].value, b);
                j[f.name] = i
            }
            return j
        },
        ...
        presence: function(a, b) {
            return b = e.extend({}, this.options, b), (b.allowEmpty ? !e.isDefined(a) : e.isEmpty(a)) ? b.message || this.message || "can't be blank" : void 0
        },
        ...
        numericality: function(a, b) {
            if (e.isDefined(a)) {
                ...
                if (e.isString(a) && b.strict) {
                    var h = "^(0|[1-9]\\d*)";
                    if (b.onlyInteger || (h += "(\\.\\d+)?"), h += "$", !new RegExp(h).test(a)) return b.message || b.notValid || this.notValid || this.message || "must be a valid number"
                }
                ...
                for (c in g)
                    if (d = b[c], e.isNumber(d) && !g[c](a, d)) {
                        var i = "not" + e.capitalize(c),
                            j = b[i] || this[i] || this.message || "must be %{type} %{count}";
                        f.push(e.format(j, {
                            count: d,
                            type: e.prettify(c)
                        }))
                    }
                return b.odd && a % 2 !== 1 && f.push(b.notOdd || this.notOdd || this.message || "must be odd"), b.even && a % 2 !== 0 && f.push(b.notEven || this.notEven || this.message || "must be even"), f.length ? b.message || f : void 0
            }
        },
